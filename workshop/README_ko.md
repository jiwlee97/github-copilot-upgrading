...
>
> 💡 다양한 프롬프트로 실험해 보며 GitHub Copilot 제안의 정확도가 어떻게 달라지는지 확인해 보세요. 예를 들어, 인라인 채팅을 사용할 때 추가 프롬프트로 응답을 세밀하게 조정할 수 있습니다.
...
`@workspace` 에이전트를 사용해 이 프로젝트가 어떻게 동작하는지 설명을 들어보세요.
...
- 기능 테스트를 활용해 집중할 _작업 단위(slice)_ 를 결정하세요.

> [!NOTE]
> 비즈니스 로직에 따라 LLM이 명확히 알지 못하는 컴포넌트가 있을 수 있으니, 설명이 완벽하지 않을 수 있습니다.
...
- 오류 메시지를 Copilot Chat에 붙여넣고 원인과 해결 방법을 물어보세요.
...
이 프로젝트에는 `pytest` 프레임워크와 명령줄 도구가 설치되어 있습니다. 단위 테스트를 실행해 오류를 확인하세요.

- 오류 출력을 Copilot에 붙여넣고(` #terminalSelection`), 도움을 요청하세요.
- Copilot의 답변을 검토하고 추가 설명이 필요하면 질문을 이어가세요.
- 아직 코드를 수정하지 마세요.

> [!NOTE]
> 오류가 바로 이해되지 않거나 Python 버전이 구식임을 나타낼 수 있습니다. Copilot에 추가 질문을 하며 오류의 원인을 파악하고 전략을 세우세요. 프로젝트가 오래된 Python 버전임을 힌트로 줄 수도 있습니다.

### 6. try/except 오류 수정하기

기능 테스트를 활용해 집중할 _작업 단위(slice)_ 를 결정하세요. 이번에는 try/except 오류를 수정합니다.

- Copilot에게 구버전 Python임을 언급하며 실제 문제 해결을 요청하세요.
...
[!NOTE]
> 모든 테스트가 통과해야 합니다. 통과하지 않으면 Copilot에 테스트 출력을 붙여넣고 도움을 요청하며 이전 단계를 반복하세요.
...
레거시 코드는 앞서 수정한 `distribute`뿐 아니라, 이제는 폐기된 `setup.py`와 `setuptools`를 사용합니다. `pyproject.toml`을 사용해 프로젝트를 패키징하세요.

- Copilot에게 `pyproject.toml` 파일 작성 방법을 물어보세요. `@workspace` 에이전트와 함께 `setup.py` 파일을 맥락에 추가하세요.
...
레거시 프로젝트에는 자동화가 없습니다. GitHub Actions를 사용해 테스트 및 프로젝트 설치를 자동화하세요. 목표는 모든 풀 리퀘스트마다 워크플로우가 실행되어 테스트와 설치를 검증하는 것입니다.

- `@workspace` 에이전트로 GitHub Actions 워크플로우 작성 방법을 물어보세요.
